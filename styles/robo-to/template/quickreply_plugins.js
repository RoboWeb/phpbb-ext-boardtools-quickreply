...
(function ($, window, document) {
  ...
  if (quickreply.settings.unchangedSubject) {
    if (quickreply.settings.hideSubjectBox) {
      ...
      $(document).ready(function () {
        quickreply.$.mainForm
          .find('input[name="subject"][type="text"]')
  ...
  if (quickreply.settings.ctrlEnterNotice) {
    quickreply.$.mainForm
      ...
      .attr("title", quickreply.language.CTRL_ENTER);
  }
  ...
  function QrHelper() {
    ...
    self._getSelectionCoords = function () {
      ...
      return {
        x: x ? x + $(document).scrollLeft() : 0,
        y: y ? y + $(document).scrollTop() : 0,
      };
    };
  ...
  function QrQuote() {
    ...
    var self = this,
      ...
      isIE =
        clientPC.indexOf("msie") !== -1 && clientPC.indexOf("opera") === -1;
  ...
  function QuickQuote() {
    ...
    function addQuickQuote(evt, element) {
      ...
      console.log({
        "quickreply_plugins.js": "addQuickQuote",
        element: $element,
        evt: evt,
      });
      ...
      setTimeout(function () {
        // Timeout prevents popup when clicking on selected text
        self._getSelection();
        self._removeDropdown();

        if (self._selection && key <= 1) {
          // If text selected && right mouse button not pressed
          var $dropdown = quickreply.style
            .quickQuoteDropdown(coordinates.x, coordinates.y)
    ...
    function handleQuickQuote(evt) {
      console.log({
        "quickreply_plugins.js": "handleQuickQuote",
      });
      var $content = $(this);
      ...
      if (!quickQuoteCancelEvent) {
        if ("onselectionchange" in document) {
          ...
          quickreply.$.qrPosts.on("mousemove", ".post-content", addQuickQuote);
          $(document.body).one("mouseup", function () {
            quickreply.$.qrPosts.off(
              "mousemove",
              ".post-content",
              addQuickQuote
            );
    ...
    self.init = function () {
      ...
      quickreply.$.qrPosts
        .on("mousedown", ".post-content", handleQuickQuote)
        .on("mouseup", ".post-content", addQuickQuote);
    };
  }
  ...
  function FullQuote() {
    ...
    function getPostContent() {
      ...
      var messageName = "decoded_p" + self._postID,
        divarea = false;
      ...
      if (divarea.innerHTML) {
        self._selection = divarea.innerHTML.replace(/<br>/gi, "\n");
        self._selection = self._selection.replace(/<br\/>/gi, "\n");
        self._selection = self._selection.replace(/&lt\;/gi, "<");
        self._selection = self._selection.replace(/&gt\;/gi, ">");
        self._selection = self._selection.replace(/&amp\;/gi, "&");
        self._selection = self._selection.replace(/&nbsp\;/gi, " ");
      ...
      self._selection = self._selection.replace(
    ...
    function addFullQuote(e, element) {
      console.log({
        "quickreply_plgins.js": "addFullQuote",
        e: e,
        element: element,
      });
      self._postID = quickreply.style.getPostId(element);
      ...
      if (quickreply.settings.fullQuote && !element.hasClass("qr-quickquote")) {
        if (
          self._selection === "" ||
          typeof self._selection === "undefined" ||
      ...
      if (self._selection !== "") {
        ...
        !quickreply.settings.fullQuoteAllowed ||
        element.hasClass("qr-quickquote")
      ) {
        e.preventDefault();
        quickreply.functions.alert(
          quickreply.language.ERROR,
          quickreply.language.NO_FULL_QUOTE
        );
      }
    }
    ...
    function setQuoteButtons(e, elements) {
      if (quickreply.settings.quickQuoteButton) {
        var quoteButtons = null,
          lastQuoteButton = null;

        if (
          !quickreply.settings.fullQuote ||
          !quickreply.settings.fullQuoteAllowed
        ) {
          ...
          !quickreply.settings.lastQuote &&
          quickreply.style.isLastPage()
        ) {
          // Style only last quote button
          quoteButtons = quickreply.style.getQuoteButtons(
            quickreply.$.qrPosts,
            "all"
          );
    ...
    function handleResponsiveClick(e) {
      e.preventDefault();
      console.log("handleResponsiveClick", $(this));
      addFullQuote(e, $(this));
    ...
    self.init = function () {
      $(window).on("load", function (e) {
        console.log(
          "quickreply_plugins.js - self.init - $(window).on(load ..."
        );
        setQuoteButtons(e, quickreply.$.qrPosts);
        quickreply.style.responsiveQuotesOnClick(
          quickreply.$.qrPosts,
      ...
      quickreply.$.qrPosts
        .on("qr_completed", function (e, elements) {
          console.log(
            "quickreply_plugins.js - quickreply.$.grPosts.on(gr_completed ..."
          );
  ...
  function QuickNick() {
    ...
    var self = this,
      ...
      nicknameSelector =
        quickreply.editor.profileLinkSelector +
        ", " +
        quickreply.editor.profileNoLinkSelector;
    ...
    function quickNickHandlePosts(e, elements) {
      ...
      elements
        .find(quickreply.editor.profileNoLinkSelector)
        .addClass("qr_quicknick_trigger");
    }
    ...
    self.insert = function ($nickname) {
      var nickname = $nickname.text(),
        color = $nickname.hasClass("username-coloured")
          ? $nickname.css("color")
          : false,
        qrColor =
          quickreply.settings.colouredNick && color
      ...
      if (!quickreply.settings.allowBBCode) {
        ...
        insert_text(
          "[ref" + qrColor + "]" + nickname + "[/ref]" + comma,
          false
        );
      }
    };
    ...
    self.init = function () {
      if (quickNickIsDropdown()) {
        ...
        quickreply.$.qrPosts
          .on("click", nicknameSelector, function (e) {
            addDropdown(e, $(this));
          })
    ...
    if (quickreply.plugins.liveSearch) {
      $(document).ready(function () {
        $("#topics_live_search")
          .parent()
          .before(
            '<li><a id="qr_quicknick_live_search" href="#qr_postform" title="' +
              quickreply.language.QUICKNICK_TITLE +
              '">' +
              quickreply.language.QUICKNICK +
