...
(function ($, window, document) {
  ...
  quickreply.style.initPreview = function () {
    console.log("quickreply.style.initPreview");
    quickreply.$.mainForm.before(
      `<div id="preview" class="post profile_hidden bg2" style="display: none;">
      <article class="postbody">
        <div>
          <h3 id="preview_title"></h3>
          <a href="#" id="preview_close"><i class="fa fa-times"></i></a>
  ...
  quickreply.style.markRead = function (elements) {
    elements
      .find(".unreadpost")
      .removeClass("unreadpost")
      .find(".icon.icon-red")
      .removeClass("icon-red")
      .addClass("icon-lightgray");
  };
  ...
  quickreply.style.formEditorElements = function (selectStandard) {
    var $qrForm = quickreply.$.mainForm,
      formatButtons = quickreply.plugins.abbc3
        ? "#abbc3_buttons"
        : "#format-buttons",
      elementsArray = [
        "#attach-panel",
    ...
    return selectStandard
      ? $elements.add($qrForm.find(".submit-buttons, #qr_captcha_container"))
      : $elements;
  };
  ...
  quickreply.style.handlePagination = function () {
    console.log({
      "quickreply_style.js": "quickreply.style.handlePagination",
    });
  ...
  quickreply.style.restoreFirstSubject = function (elements, tempContainer) {
    if (
      quickreply.settings.hideSubject &&
      !elements.find(".post-container").length
    ) {
      tempContainer
        .find(".post .postbody div h3")
        .first()
        .addClass("first")
        .css("display", "");
    }
  ...
  quickreply.style.bindPagination = function () {
    if (quickreply.settings.saveReply) {
      $(".action-bar-top, .action-bar-bottom")
        .find('.pagination a:not(.dropdown-trigger, .mark[href="#unread"])')
    ...
    $(".action-bar-top, .action-bar-bottom")
      .find('.pagination a.mark[href="#unread"]')
      .click(function (event) {
        event.preventDefault();
        var $unreadPosts = $(".unreadpost");
        quickreply.functions.softScroll(
          $unreadPosts.length ? $unreadPosts.first() : quickreply.$.qrPosts
        );
      });
    ...
    $(".action-bar-top, .action-bar-bottom")
      .find(".pagination .dropdown-trigger")
      .click(function () {
  ...
  quickreply.style.initPagination = function () {
    $(window).on("load", function () {
      $(".action-bar-top, .action-bar-bottom")
        .find(".pagination")
        .find(".page-jump-form :button")
        .unbind("click");
      $(".action-bar-top, .action-bar-bottom")
        .find(".pagination")
        .find(".page-jump-form input.inputbox")
        .off("keypress");
  ...
  quickreply.style.getPostId = function (element) {
    let postId = element
      .parents(".postbody > div")
      .attr("id")
      .replace("post_content", "");
    console.log({
      "quickreply_style.js": "quickreplay.style.getPostId",
      pstId: postId,
    });
    return postId;
  };
  ...
  quickreply.style.getQuoteButtons = function (elements, type) {
    ...
    console.log({
      "quickreply_style.js": "quickreply.style.getQuoteButtons",
      conteiner: container,
      buttons: buttons,
    });
    return !type
  ...
  quickreply.style.responsiveQuotesOnClick = function (elements, fn) {
    console.log({
      quoteButtons: elements.find(".post-actions a").has(".fa-quote-left"),
    });
    elements.find(".post-actions a").has(".fa-quote-left").on("click", fn);
  };
  ...
  quickreply.style.setSkipResponsiveForQuoteButtons = function (elements) {
    quickreply.style
      .getQuoteButtons(elements)
      .closest("li")
      .attr("data-skip-responsive", "true");
  };
  ...
  quickreply.style.isLastPage = function () {
    ...
    return (
      paginationContainer.find("li").last().hasClass("active") ||
  ...
  quickreply.style.setQuickQuoteButton = function (elements) {
    console.log(
      "quickreply_style.js -  quickreply.style.setQuickQuoteButton, elements=",
      elements
    );
    elements
      .addClass("qr-quickquote")
      .attr("title", quickreply.language.QUICKQUOTE_TITLE)
      .children("span")
      .text(quickreply.language.QUICKQUOTE_TEXT);
  };
  ...
  quickreply.style.removeQuickQuoteButton = function (elements) {
    console.log("quickreply.style.removeQuickQuoteButton");
    elements
      .removeClass("qr-quickquote")
      .attr("title", quickreply.language.REPLY_WITH_QUOTE)
      .children("span")
      .text(quickreply.language.BUTTON_QUOTE);
  };
   ...
   * @returns {jQuery}
   */
  quickreply.style.createDropdown = function (
    contentRows,
    pageX,
    pageY,
    appendClass
    ...
    var dropdownStyle = "top: " + (pageY + 8) + "px; ",
      pointerStyle = "";
    if (pageX > 184) {
      dropdownStyle +=
        "margin-right: 0; left: auto; right: " +
        ($("body").width() - pageX - 20) +
        "px;";
    ...
    return $(
      '<div class="dropdown qr_dropdown' +
        appendClass +
        '" style="' +
        dropdownStyle +
  ...
  quickreply.style.quickQuoteDropdown = function (pageX, pageY) {
    ...
    return quickreply.style.createDropdown(
      listElements,
      pageX,
      pageY,
      "qr_quickquote"
  ...
  quickreply.style.quickNickDropdown = function (
    ...
    console.log("quickreply.style.quickNickDropdown");
    var listElements = [
      quickreply.functions.makeLink({
        href: "#qr_postform",
