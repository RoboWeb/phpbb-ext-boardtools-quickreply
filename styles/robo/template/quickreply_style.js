...
;(function($, window, document) {
	...
	quickreply.style.initPreview = function() {
		quickreply.$.mainForm.before('<div id="preview" class="post" style="display: none;"><article class="postbody"><h3></h3><i id="preview_close" class="fa fa-times"></i><div class="post-content"></div></article></div>');
	};
	...
	quickreply.style.bindPagination = function() {
		if (quickreply.settings.saveReply) {
			$('.pagination a:not(.dropdown-trigger, .mark[href="#unread"])').click(function(event) {
		...
		$('.pagination .page-jump-form input.inputbox').on('keypress', function(event) {
	...
	quickreply.style.createDropdown = function(contentRows, pageX, pageY, appendClass) {
		...
		return $('<div class="dropdown-box qr_dropdown' + appendClass + '" style="' + dropdownStyle + '"><div class="pointer" style="' + pointerStyle + '"><div class="pointer-inner"></div></div><ul class="dropdown-contents dropdown-nonscroll"><li>' + contentRows.join('</li><li>') + '</li></ul></div>').appendTo('body');
	};
