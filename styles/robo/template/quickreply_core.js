...
;(function($, window, document) {
	...
	function QrForm() {
		...
		this.updateFields = function(qrFields) {
			var formSubmitButtons = self.$.children().children().children('.quickreply-buttons');
		...
		function exitExtended() {
			...
			$('#qr_action_box, #qr_text_action_box, #qr_captcha_container, .quickreply-buttons').show();
		...
		this.exitFullscreen = function() {
			...
			$('.qr_fixed_form').animate({
				...
				setFormWidth();
			}).removeClass('qr_fullscreen_form').removeAttr('style');
	...
	function Preview() {
		...
		this.set = function(options) {
			...
			$preview.finish().css('display', ops.display)
				...
				.find('.post-content').html(ops.content).end();
			if (quickreply.settings.attachBox) {
		...
		this.init = function() {
			if (quickreply.settings.ajaxSubmit) {
				...
				quickreply.$.mainForm.attr('data-ajax', 'qr_ajax_submit').submit(function() {
					...
					var $clickedButton = $(this).find('button[type="submit"][data-clicked]');

					// Fix for phpBB 3.1.9
					if (!$clickedButton.length) {
						$clickedButton = $(this).find('button[name="post"]').attr('data-clicked', 'true');
					}
		...
		this.submitCallback = function(res) {
			...
			switch (res.status) {
				...
				case "outdated_form":
					...
					var $clickedButton = quickreply.$.mainForm.find('button[data-clicked="true"]');
			...
			/* Fix for phpBB 3.1.9 */
			quickreply.$.mainForm.find('button[data-clicked]').removeAttr('data-clicked');
		};
	}
